...
{
	function date_convert($date, $formatFrom = 'mm/dd/yyyy', $formatTo = 'Y-m-d')
	{
...
{
	function is_valid_date($date, $desired_format) {
		
		if (strpos($desired_format, '/') !== false) {
			list($f[0], $f[1], $f[2]) = explode('/', $desired_format);
		...
		return checkdate($d[array_search("mm",$f)], $d[array_search("dd",$f)], $d[array_search("yyyy",$f)]);
	}
}
...
if ( ! function_exists('nicetime_lang'))
{
	...
	{
		$ptime = is_numeric($ptime) ? $ptime : strtotime($ptime);						
