jQuery(document).ready(function () {
  ...
  jQuery(function ($) { 
    $("select[name=ccy_id]").val(getURLParam("ccy_id") ?? 1);
    $("select[name=ccy_id]").on("change", function (e) {
      // e.preventDefault();
      // alert(this.value);
      // console.log(insertURLParam('ccy_id', this.value));
      // let param = insertURLParam("ccy_id", this.value);
      // let url = getUrl("frontend/summary") + param;
      // console.log(url);
      document.location.search = insertURLParam("ccy_id", this.value);

      // window.location.href = getUrl("frontend/summary");
      // $.get(getUrl("frontend/summary"), { "ccy_id": this.value })
      //   .done(function (data) {
      //     console.log(data);
      //     alert(data);
